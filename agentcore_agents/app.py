...
11. **Link source documents only for visual content.** Most of your response is fully conveyed in words and must NOT carry a link. Add an inline citation marker ONLY to a section that genuinely depends on a diagram, chart, picture, model, or visual layout that the teacher would need to open the original document to see and understand. Use exactly this format: `[[src:FILENAME]]` — the exact source filename from the "[Retrieved from: ...]" header (e.g. `[[src:TN_M7_L3_en.pdf]]`), placed once at the end of that section's heading or final sentence. Do NOT add a marker to sections that are fully explained in text — such as lists of key questions, key principles, common mistakes, or named manipulatives. If in doubt, leave it out: a link should mean "there is a visual here worth opening", not "this came from a document". Most responses will have zero or one marker. Do not explain or alter the marker.
